...
export default function UserSelection({ selectUser }) {
  ...
  useEffect(() => {
    ...
    const storedResults = localStorage.getItem('coding-round-master_quizResults');
    if (storedResults) {
  ...
  const addUser = (name) => {
    ...
    localStorage.setItem('coding-round-master_quizUserId', newUserId);
    ...
    localStorage.setItem('coding-round-master_quizResults', JSON.stringify(updatedUsers));
