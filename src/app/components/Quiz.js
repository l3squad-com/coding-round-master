...
export default function Quiz({ quizType, user, resetQuiz }) {
  ...
  const finishQuiz = () => {
    ...
    const currentUserId = localStorage.getItem('coding-round-master_quizUserId');

    if (currentUserId) {
      // Get existing quiz results from local storage
      const storedResults = JSON.parse(localStorage.getItem('coding-round-master_quizResults') || '[]');
      ...
      localStorage.setItem('coding-round-master_quizResults', JSON.stringify(updatedResults));
    }
  };
